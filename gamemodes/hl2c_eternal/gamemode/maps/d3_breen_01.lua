...
function hl2cThink()
	if game.GetGlobalState("super_phys_gun") == GLOBAL_ON then
		...
		for _,ent in ipairs(ents.FindByClass("weapon_*")) do
			if IsValid(ent) and ent:IsWeapon() and ent:GetClass() ~= "weapon_physcannon" and (!IsValid(ent:GetOwner()) or (IsValid(ent:GetOwner()) and ent:GetOwner():IsPlayer())) and not ent.CanUseInCitadel then
				ent:Remove()
			end
		end
	end
