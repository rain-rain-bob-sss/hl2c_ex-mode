-- Lambda Gamemode's sv_transition.lua
-- https://github.com/ZehMatt/Lambda/blob/develop/gamemode/sv_transition.lua
...
-- lambda's GM:PreChangeLevel
function GM:OnChangeLevelTrigger(trigger, activator, map, landmark, restart, mapaddFile)
	landmark = landmark or trigger.Landmark

	local playersInTrigger = table.Copy(trigger.m_TouchingEntities)
	DbgPrint("GM:PreChangelevel", trigger, activator, map, landmark, playersInTrigger, restart)

	util.SetPData("HL2CE_PData" .. --[[lambda_instance_id:GetString()]] "", "PrevMap", self:GetCurrentMap())
	util.SetPData("HL2CE_PData" .. --[[lambda_instance_id:GetString()]] "", "NextMap", map)
	util.SetPData("HL2CE_PData" .. --[[lambda_instance_id:GetString()]] "", "Landmark", landmark)
	util.SetPData("HL2CE_PData" .. --[[lambda_instance_id:GetString()]] "", "Changelevel", restart and "0" or "1")

	self:TransitionToLevel(activator, map, landmark, playersInTrigger, restart, mapaddFile)
end

function GM:TransitionToLevel(activator, map, landmark, playersInTrigger, restart, mapaddFile)
	...
	if not IsValid(landmarkEnt) then
		...
		for k, v in pairs(playerTable) do
			...
			if restart == true or not playersInTrigger[ply] then
	...
	for _, ent in ents.Iterator() do
		...
		if bit.band(caps, FCAP_NOTIFY_ON_TRANSITION) ~= 0 then
			if transitionMap[ent] == true then
				ent:Fire("OutsideTransition")
			else
				ent:Fire("InsideTransition")
			end
		end
	end

	util.SetPData("HL2CE_PData" .. --[[_instanceID]] "", "TransitionData", util.TableToJSON(transitionData))
end
...
function GM:InitializeTransitionData()
	...
	if self.IsChangeLevel == true then
		...
		local encodedTransitionData = util.GetPData("HL2CE_PData" .. --[[instanceID]] "", "TransitionData", nil)
	...
	DbgPrint("TransitionData contains ")
	DbgPrint("  Player objects: " .. tostring(table.Count(transitionData.Players or {})))
	DbgPrint("  World objects: " .. tostring(table.Count(transitionData.Objects or {})))
	DbgPrint("  Global states: " .. tostring(table.Count(transitionData.GlobalStates or {})))
	if transitionData.MapAddFile and #transitionData.MapAddFile > 0 then
		DbgPrint("  Map Add File: " .. tostring(transitionData.MapAddFile))
	else
		DbgPrint("  No Map Add File")
	end

	--PrintTable(self.TransitionData)
	util.RemovePData("HL2CE_PData" .. --[[instanceID]] "", "TransitionData")
end
...
function GM:TransitionNearbyObjects(activator, landmarkEnt, transitionTriggers, objectTable, playerTable, playersInTrigger)
	...
	for _, ent in ents.Iterator() do
		if bit.band(ent:ObjectCaps(), FCAP_NOTIFY_ON_TRANSITION) == 0 then continue end

		if table.HasValue(initialObjectList, ent) == false then
			ent:Input("OutsideTransition", activator, activator, nil)
			--DbgPrint("Notifying " .. tostring(v) .. " outside of transitioning")
		else
			ent:Input("InsideTransition", activator, activator, nil)
...
function GM:GetTransitionList(landmarkEnt, transitionTriggers, objectTable, playerTable, playersInTrigger)
	...
	for _, ply in player.Iterator() do
		...
		if ply:InVehicle() and playersInTrigger[ply] then
			table.insert(objects, ply:GetVehicle())
		end
	end
	...
	for _, ent in ents.Iterator() do
		if self:ShouldTransitionObject(ent, playersInTrigger) == false then
			continue
		end

		if ent.ForceTransition ~= true then
			--DbgPrint("Testing PVS on: " .. tostring(v))
			if landmarkEnt:TestPVS(ent) == false then
				continue
			end
		else
		...
		if checkVolumes and ent.ForceTransition ~= true and self:InTransitionVolume(transitionTriggers, ent) == false then
...
function GM:ShouldTransitionObject(obj, playersInTrigger)
	...
	if obj:IsVehicle() and transition == false then
		...
		if IsValid(driver) and driver:IsPlayer() then
			if playersInTrigger[driver] then
...
function GM:InTransitionVolume(volumes, obj)
	...
	for _, volume in pairs(volumes) do
		...
		local volMins, volMaxs = volume:OBBMins(), volume:OBBMaxs()
		volMins:Add(volPos)
		volMaxs:Add(volPos)
...
function GM:SerializePlayerData(landmarkEnt, ply, playersInTrigger)
	...
	if playersInTrigger[ply] and ply:Alive() == true then
...
function GM:InitializeCurrentLevel()
	...
	local changelevel = tobool(util.GetPData("HL2CE_PData" .. --[[lambda_instance_id:GetString()]] "", "Changelevel", "0"))
	local prevMap = util.GetPData("HL2CE_PData" .. --[[lambda_instance_id:GetString()]] "", "PrevMap", nil)
	local targetMap = util.GetPData("HL2CE_PData" .. --[[lambda_instance_id:GetString()]] "", "NextMap", nil)
	local landmark = util.GetPData("HL2CE_PData" .. --[[lambda_instance_id:GetString()]] "", "Landmark", nil)
	...
	util.RemovePData("HL2CE_PData" .. --[[lambda_instance_id:GetString()]] "", "Changelevel")
	util.RemovePData("HL2CE_PData" .. --[[lambda_instance_id:GetString()]] "", "Landmark")
	util.RemovePData("HL2CE_PData" .. --[[lambda_instance_id:GetString()]] "", "PrevMap")
...
-- TODO: timeout for this data and restore in case of level change crash
GM.BackupTransitionData = {}
hook.Add("PlayerFullyConnected", "hl2ce_transition", function(ply)
	...
	local Players = GAMEMODE.TransitionData.Players
	for k, v in pairs(Players) do
		if v[key] == val then
			DbgPrint("Wiping transition data for " .. tostring(ply))
			GAMEMODE.BackupTransitionData[val] = v
			Players[k] = nil
...
local sv_timeout = GetConVar"sv_timeout"
GM.Players = {}
GM.Connecting = {}
function GM:TransferPlayers()
	...
	if self.TransitionData ~= nil and self.TransitionData.Players ~= nil then
		...
		for _, data in pairs(self.TransitionData.Players) do
			...
			local timeout = sv_timeout:GetFloat() --self:GetSetting("connect_timeout")
		...
		--    hook.Run("NotifyPlayerListChanged", k)
		--end
	end
end
...
function GM:CreateTransitionObjects()
	...
	-- R
	local entryLandmark = self.EntryLandmark
	local landmarkEntities = {}
	if entryLandmark ~= nil then
		DbgPrint("Entry Landmark: " .. entryLandmark)
		for _, ent in pairs(ents.FindByName(entryLandmark)) do
			if ent:GetClass() ~= "trigger_transition" then
				continue
			end
			DbgPrint("Landmark Entity:", ent)
			local touchingObjects = ent:GetTouching()
	...
	local function findByGlobalName(name)
		for i, ent in ents.Iterator() do
			local globalName = ent:GetNWString("GlobalName", ent:GetInternalVariable("globalname"))
	...
	local function findByName(name)
		for i, ent in pairs(landmarkEntities) do
...
function GM:PostLoadTransitionData()
	...
	if not IsValid(landmarkEnt) then
		if not table.IsEmpty(self.TransitionData.Players) or not table.IsEmpty(self.TransitionData.Objects) then
