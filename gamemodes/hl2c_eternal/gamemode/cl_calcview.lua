...
function GM:CalcView( ply, origin, angles, fov, znear, zfar )
	...
	if IsValid(Weapon) then
		...
		if func then view.origin, view.angles, view.fov = func(Weapon, ply, origin * 1, angles * 1, fov) end
	end

	-- Client thirdperson
	if hl2c_cl_thirdperson:GetBool() and ply:Alive() and not ply:InVehicle() and (ply:GetViewEntity() == ply) then
