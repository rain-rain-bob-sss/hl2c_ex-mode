...
function SWEP:Initialize()
	...
	timer.Create("hl2ce_medkit_ammo" .. self:EntIndex(), 2, 0, function() if IsValid(self) and (self:Clip1() < self:MaxAmmo()) then self:SetClip1(math.min(self:Clip1() + self:GetRegenAmount(), self:MaxAmmo())) end end)
end
...
local dist = 72
local ffunc = function(ent) return (ent:IsNPC() or ent:IsPlayer()) and ent:Health() < ent:GetMaxHealth() end
function SWEP:PrimaryAttack()
	...
	local tr = util.TraceHull({
		...
		filter = function(e) return e ~= self:GetOwner() and ffunc(e) end,
		mins = hullmin,
		maxs = hullmax
	})
	...
	if IsValid(ent) then need = math.min(ent:GetMaxHealth() - ent:Health(), self.HealAmount + (self.HealAmount * ((GAMEMODE.EndlessMode and 0.05 or 0.02) * self:GetOwner():GetSkillAmount("Medical")))) end
	if IsValid(ent) and self:Clip1() >= need and (ent:IsPlayer() or ent:IsNPC()) and ent:Health() < ent:GetMaxHealth() then
		...
		if self:GetOwner():HasPerkActive("medkit_charging") and SERVER and ent:IsPlayer() then ent:SetArmor(math.min(ent:GetMaxArmor(), ent:Armor() + need * 0.5)) end
		if ent:IsPlayer() then
			self:GetOwner():SetHealth(math.min(self:GetOwner():GetMaxHealth(), self:GetOwner():Health() + need * 0.25))
			self:GetOwner():GiveXP(need * 0.35)
		elseif ent:IsFriendlyNPC() then
			self:GetOwner():SetHealth(math.min(self:GetOwner():GetMaxHealth(), self:GetOwner():Health() + need * 0.2))
			self:GetOwner():GiveXP(need * 0.28)
		end
...
function SWEP:SecondaryAttack()
	...
	if IsValid(ent) and self:Clip1() >= need and (ent:IsPlayer() or ent:IsNPC()) and ent:Health() < ent:GetMaxHealth() then
		self:TakePrimaryAmmo(need)
		ent:SetHealth(math.min(ent:GetMaxHealth(), ent:Health() + need))
		if ent:HasPerkActive("medkit_charging") and SERVER then ent:SetArmor(math.min(ent:GetMaxArmor(), ent:Armor() + need * 0.5)) end
		ent:EmitSound(HealSound)
		self:SendWeaponAnim(ACT_VM_PRIMARYATTACK)
...
function SWEP:DoDrawCrosshair(x, y)
	...
	if not follow then
		...
		blahblahblahcos = blahblahblahcos + FrameTime() * 5
	end
